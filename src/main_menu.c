 ...
 * Task_NewGameBirchSpeech_StartBirchSkittyPlatformFade
 * Task_NewGameBirchSpeech_StartBirchSkittyPlatformFade
 ...
 * Task_NewGameBirchSpeech_ReshowBirchSkitty
 ...
 *  - Advances to Task_NewGameBirchSpeechSub_WaitForSkitty
 * Task_NewGameBirchSpeechSub_WaitForSkitty
...
static void Task_NewGameBirchSpeechSub_WaitForSkitty(u8);
static void Task_NewGameBirchSpeech_StartBirchSkittyPlatformFade(u8);
...
static void Task_NewGameBirchSpeech_ReshowBirchSkitty(u8);
...
{
    u8 spriteId = gTasks[sBirchSpeechMainTaskId].tSkittySpriteId;
    ...
    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, 0x0000FFFF, SPECIES_SKITTY);
    gTasks[taskId].func = Task_NewGameBirchSpeechSub_WaitForSkitty;
    gTasks[sBirchSpeechMainTaskId].tTimer = 0;
}

static void Task_NewGameBirchSpeechSub_WaitForSkitty(u8 taskId)
{
    s16 *data = gTasks[taskId].data;
    struct Sprite *sprite = &gSprites[gTasks[sBirchSpeechMainTaskId].tSkittySpriteId];
...
static void Task_NewGameBirchSpeech_AndYouAre(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        ...
        AddTextPrinterForMessage(1);
        gTasks[taskId].func = Task_NewGameBirchSpeech_StartBirchSkittyPlatformFade;
    }
}

static void Task_NewGameBirchSpeech_StartBirchSkittyPlatformFade(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        gSprites[gTasks[taskId].tSkittySpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_StartPlayerFadeIn(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[gTasks[taskId].tSkittySpriteId].invisible = TRUE;
        if (gTasks[taskId].tTimer)
        {
            gTasks[taskId].tTimer--;
        }
...
static void Task_NewGameBirchSpeech_StartNamingScreen(u8 taskId)
{
    ...
    {
        FreeAllWindowBuffers();
        FreeAndDestroyMonPicSprite(gTasks[taskId].tSkittySpriteId);
...
static void Task_NewGameBirchSpeech_SlidePlatformAway2(u8 taskId)
{
    ...
    {
        gTasks[taskId].func = Task_NewGameBirchSpeech_ReshowBirchSkitty;
    }
}

static void Task_NewGameBirchSpeech_ReshowBirchSkitty(u8 taskId)
{
    u8 spriteId;
    ...
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        spriteId = gTasks[taskId].tSkittySpriteId;
        gSprites[spriteId].x = 100;
        gSprites[spriteId].y = 75;
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_WaitForSpriteFadeInAndTextPrinter(u8 taskId)
{
    ...
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        gSprites[gTasks[taskId].tSkittySpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        if (!RunTextPrintersAndIsPrinter0Active())
        {
            gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
            gSprites[gTasks[taskId].tSkittySpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static u8 NewGameBirchSpeech_CreateSkittySprite(u8 a, u8 b)
{
    return CreatePicSprite2(SPECIES_SKITTY, SHINY_ODDS, 0, 1, a, b, 14, -1);
}
...
static void AddBirchSpeechObjects(u8 taskId)
{
    ...
    skittySpriteId = NewGameBirchSpeech_CreateSkittySprite(100, 0x4B);
    gSprites[skittySpriteId].callback = SpriteCB_Null;
    gSprites[skittySpriteId].oam.priority = 0;
    gSprites[skittySpriteId].invisible = TRUE;
    gTasks[taskId].tSkittySpriteId = skittySpriteId;
