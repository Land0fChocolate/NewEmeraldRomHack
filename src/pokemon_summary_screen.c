...
static void Task_HandleInput(u8 taskId)
{
    if (MenuHelpers_CallLinkSomething() != TRUE && !gPaletteFade.active)
    {
        ...
        else if (JOY_NEW(A_BUTTON) && !GetMonData(&sMonSummaryScreen->currentMon, MON_DATA_IS_EGG))
        {
...
static void PrintEggState(void)
{
    ...
    PrintTextOnWindow(AddWindowFromTemplateList(sPageInfoTemplate, PSS_DATA_WINDOW_INFO_ABILITIES), text, 0, 16, 0, 0);
}
        ...
        if (summary->moves[i] != MOVE_NONE) {
            if (summary->moves[i] == MOVE_HIDDEN_POWER) {
                ...
                if (type >= TYPE_MYSTERY)
                    type++;
                type |= 0xC0;
                SetTypeSpritePosAndPal(type & 0x3F, 85, 32 + (i * 16), i + SPRITE_ARR_ID_MOVE_TYPE);
            } else {
        ...
        if (sMonSummaryScreen->currPageIndex == PSS_PAGE_BATTLE_MOVES)
            if (sMonSummaryScreen->newMove == MOVE_HIDDEN_POWER) {
                ...
                if (type >= TYPE_MYSTERY)
                    type++;
                type |= 0xC0;
                SetTypeSpritePosAndPal(type & 0x3F, 85, 96, SPRITE_ARR_ID_MOVE_TYPE + 4);
            } else {
...
static u8 LoadMonGfxAndSprite(struct Pokemon *mon, s16 *state)
{
    ...
    case 0:
        if (gMain.inBattle)
        {
            HandleLoadSpecialPokePic(TRUE, gMonSpritesGfxPtr->sprites.ptr[1], summary->species2, summary->pid);
        }
        else
        {
            if (gMonSpritesGfxPtr != NULL)
            ...
            {
                HandleLoadSpecialPokePic(TRUE, sub_806F4F8(0, 1), summary->species2, summary->pid);
            }
        }
        (*state)++;
        return 0xFF;
