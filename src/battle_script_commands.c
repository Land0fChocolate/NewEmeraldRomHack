...
static const u8 sForbiddenMoves[MOVES_COUNT] =
{
    ...
    [MOVE_TRICK] = FORBIDDEN_METRONOME | FORBIDDEN_ASSIST | FORBIDDEN_COPYCAT | FORBIDDEN_INSTRUCT,
...
static void Cmd_attackcanceler(void)
{
    ...
    && HasAbility(ABILITY_PARENTAL_BOND, abilities)
    ...
    else if (HasAbility(ABILITY_MAGIC_BOUNCE, GetBattlerAbilities(gBattlerTarget))
             && gBattleMoves[gCurrentMove].flags & FLAG_MAGIC_COAT_AFFECTED
             && !gProtectStructs[gBattlerAttacker].usesBouncedMove)
    {
...
u32 GetTotalAccuracy(u32 battlerAtk, u32 battlerDef, u32 move, u32 atkHoldEffect, u32 defHoldEffect)
{
    u32 calc, moveAcc;
...
static void Cmd_accuracycheck(void)
{
    ...
    if (move == NO_ACC_CALC_CHECK_LOCK_ON)
    {
        ...
        || HasAbility(ABILITY_SKILL_LINK, GetBattlerAbilities(gBattlerAttacker)))))
    {
        ...
        else
        {
            ...
            case MOVE_EFFECT_RECOIL_HP_25: // Struggle
                ...
                if (gBattleMoveDamage == 0)
                    gBattleMoveDamage = 1;
                if (HasAbility(ABILITY_PARENTAL_BOND, GetBattlerAbilities(gEffectBattler)))
                    gBattleMoveDamage *= 2;
            ...
            case MOVE_EFFECT_FEINT:
                if (IS_BATTLER_PROTECTED(gBattlerTarget))
                {
                    ...
                    gProtectStructs[gBattlerTarget].singularityEffect = FALSE;
...
{
    const u8 *BS_ptr;
    u32 DeoxysBossBattleState = VarGet(VAR_DEOXYS_BOSS_BATTLE_STATE);
    u8 i;
            ...
            if (DeoxysBossBattleState > 0 && GetBattlerSide(gActiveBattler) == B_SIDE_OPPONENT)
            {
                ...
                switch (DeoxysBossBattleState)
                {
                    ...
                    case 3: //transform to attack
                        ...
                        SetBattleMonMoveSlot(&gBattleMons[gActiveBattler], MOVE_PSYSHOCK, 0);
                        SetBattleMonMoveSlot(&gBattleMons[gActiveBattler], MOVE_DARK_PULSE, 1);
                        SetBattleMonMoveSlot(&gBattleMons[gActiveBattler], MOVE_ENERGY_BALL, 2);
                        SetBattleMonMoveSlot(&gBattleMons[gActiveBattler], MOVE_FLASH_CANNON, 3);
    ...
    do
    {
        ...
        case MOVEEND_PROTECT_LIKE_EFFECT:
            if (gProtectStructs[gBattlerAttacker].touchedProtectLike)
            {
                ...
                else if (gProtectStructs[gBattlerTarget].singularityEffect)
                {
                    gProtectStructs[gBattlerAttacker].touchedProtectLike = FALSE;
                    i = gBattlerAttacker;
                    gBattlerAttacker = gBattlerTarget;
        ...
        case MOVEEND_MAGICIAN:
            if (HasAbility(ABILITY_MAGICIAN, GetBattlerAbilities(gBattlerAttacker))
...
static void Cmd_switchindataupdate(void)
{
    ...
    memcpy(gBattleMons[gActiveBattler].abilities, gBaseStats[gBattleMons[gActiveBattler].species].abilities, sizeof(gBattleMons[gActiveBattler].abilities));
...
static void RecalcBattlerStats(u32 battler, struct Pokemon *mon)
{
    ...
    memcpy(gBattleMons[battler].abilities, GetMonAbilities(mon), sizeof(gBattleMons[battler].abilities));
...
static void Cmd_setprotectlike(void)
{
    ...
    if (sProtectSuccessRates[gDisableStructs[gBattlerAttacker].protectUses] >= Random() && notLastTurn)
    {
        if (!gBattleMoves[gCurrentMove].argument) // Protects one mon only.
        {
            ...
            else if (gCurrentMove == MOVE_SINGULARITY)
            {
                gProtectStructs[gBattlerAttacker].singularityEffect = TRUE;
                gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_PROTECTED_ITSELF;
            }
        ...
        for (i = 0; i < PARTY_SIZE; i++)
        {
            ...
            if (species != SPECIES_NONE && species != SPECIES_EGG)
            {
                ...
                if (gBattlerPartyIndexes[gBattlerAttacker] == i)
                    ...
                    memcpy(abilities, GetAbilitiesBySpecies(species), sizeof(abilities));
...
static void Cmd_handleballthrow(void)
{
    ...
    else
    {
        ...
        else // mon may be caught, calculate shakes
        {
            ...
            gBattleSpritesDataPtr->animationData->isCriticalCapture = FALSE;    //initialize
