...
static const u8 sRubyFacilityClassToEmerald[RS_FACILITY_CLASSES_COUNT - 2][2] =
{
    ...
    {RS_FACILITY_CLASS_CHAMPION, FACILITY_CLASS_CHAMPION_STEVEN},
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        for (j = 0; j < 2; j++)
        {
            while (1)
            {
                ...
                if (j % 2 != 0 && gFacilityTrainerMons[gSaveBlock2Ptr->frontier.trainerIds[r10 - 1]].item == gFacilityTrainerMons[monId].item)
                    continue;
